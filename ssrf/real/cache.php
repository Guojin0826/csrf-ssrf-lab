...
// 处理请求
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cache_url'])) {
    $url = trim($_POST['cache_url']);
...
<head>
    ...
    <title>Cache Preload - 缓存预加载服务</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .cache-stats { display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; margin-top: 20px; }
        .stat-item { background: #f8f9fa; padding: 15px; border-radius: 6px; text-align: center; }
        .stat-item h4 { color: #667eea; font-size: 2em; margin-bottom: 5px; }
        .stat-item p { color: #666; font-size: 0.9em; }
    </style>
</head>
<body>
    <div class="header">
        <h1>💾 Cache Preload - 缓存预加载服务</h1>
        <div class="nav-links">
            ...
            <div class="card-body">
                ...
                <form method="POST">
                    <div class="form-group">
                        <label>预加载URL</label>
                        <input type="text" name="cache_url" placeholder="https://example.com/page.html" value="<?php echo htmlspecialchars($_POST['cache_url'] ?? ''); ?>">
                    </div>
                    <button type="submit" class="btn">开始预加载</button>
                </form>
                ...
                <div class="tips-box">
                    <h4>💡 SSRF攻击Payload</h4>
                    <ul>
                        <li>http://127.0.0.1:6379/INFO - Redis信息</li>
                        <li>http://internal-service:8080/ - 内网服务</li>
                        <li>file://./config/database.yml - 配置文件</li>
                    </ul>
                </div>
