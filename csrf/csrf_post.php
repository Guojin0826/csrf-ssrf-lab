<?php
/**
 * CSRF POST型攻击演示
 */
session_start();

// 检查登录
if (!isset($_SESSION['csrf_demo_user'])) {
    header('Location: index.php');
    exit;
}

$username = $_SESSION['csrf_demo_user'];

// 处理POST请求修改信息（漏洞版本）
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_vulnerable') {
    $nickname = $_POST['nickname'] ?? '';
    $phone = $_POST['phone'] ?? '';
    
    $_SESSION['csrf_demo_users'][$username]['nickname'] = $nickname;
    $_SESSION['csrf_demo_users'][$username]['phone'] = $phone;
    $success = '信息修改成功（漏洞版本 - 无CSRF防护）';
}

// 处理POST请求修改信息（安全版本）
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_safe') {
    $token = $_POST['csrf_token'] ?? '';
    $nickname = $_POST['nickname'] ?? '';
    $phone = $_POST['phone'] ?? '';
    
    // 验证Token
    if (isset($_SESSION['csrf_token']) && $token === $_SESSION['csrf_token']) {
        $_SESSION['csrf_demo_users'][$username]['nickname'] = $nickname;
        $_SESSION['csrf_demo_users'][$username]['phone'] = $phone;
        $success = '信息修改成功（安全版本 - CSRF Token验证通过）';
    } else {
        $error = 'CSRF Token验证失败，请求被拦截！';
    }
}

// 生成CSRF Token
$_SESSION['csrf_token'] = bin2hex(random_bytes(32));

$user_info = $_SESSION['csrf_demo_users'][$username];
?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>POST型CSRF攻击演示</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { 
            font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
            background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
            min-height: 100vh;
            padding: 20px;
        }
        .container { max-width: 1000px; margin: 0 auto; }
        .header {
            background: white;
            border-radius: 12px;
            padding: 25px;
            margin-bottom: 20px;
            box-shadow: 0 5px 20px rgba(0,0,0,0.1);
        }
        .header h1 {
            color: #764ba2;
            font-size: 1.8em;
            margin-bottom: 10px;
        }
        .header p { color: #666; }
        .nav-bar {
            background: white;
            border-radius: 12px;
            padding: 15px 20px;
            margin-bottom: 20px;
            display: flex;
            justify-content: space-between;
            align-items: center;
            box-shadow: 0 5px 20px rgba(0,0,0,0.1);
        }
        .nav-links { display: flex; gap: 10px; }
        .nav-links a {
            padding: 8px 16px;
            background: #f8f9fa;
            color: #764ba2;
            text-decoration: none;
            border-radius: 6px;
            transition: all 0.3s;
            font-size: 0.9em;
        }
        .nav-links a:hover, .nav-links a.active {
            background: #764ba2;
            color: white;
        }
        .btn {
            padding: 10px 20px;
            border: none;
            border-radius: 6px;
            cursor: pointer;
            text-decoration: none;
            font-size: 0.9em;
            transition: all 0.3s;
            display: inline-block;
        }
        .btn-primary { background: #764ba2; color: white; }
        .btn-success { background: #67c23a; color: white; }
        .btn-danger { background: #e6a23c; color: white; }
        .btn:hover { opacity: 0.9; transform: translateY(-1px); }
        
        .content {
            background: white;
            border-radius: 12px;
            padding: 30px;
            box-shadow: 0 5px 20px rgba(0,0,0,0.1);
        }
        .section { margin-bottom: 30px; }
        .section h2 {
            color: #333;
            margin-bottom: 15px;
            padding-bottom: 10px;
            border-bottom: 2px solid #764ba2;
        }
        .section p { color: #666; line-height: 1.8; margin-bottom: 15px; }
        
        .alert {
            padding: 15px;
            border-radius: 8px;
            margin-bottom: 20px;
        }
        .alert-success { background: #f0f9ff; color: #67c23a; border: 1px solid #c2e7b0; }
        .alert-danger { background: #fef0f0; color: #f56c6c; border: 1px solid #fde2e2; }
        .alert-info { background: #ecf5ff; color: #409eff; border: 1px solid #d9ecff; }
        .alert-warning { background: #fdf6ec; color: #e6a23c; border: 1px solid #faecd8; }
        
        .code-block {
            background: #2d2d2d;
            color: #f8f8f2;
            padding: 15px;
            border-radius: 6px;
            font-family: 'Courier New', monospace;
            font-size: 0.85em;
            overflow-x: auto;
            margin: 15px 0;
        }
        
        .demo-box {
            background: #f8f9fa;
            border-radius: 8px;
            padding: 20px;
            margin: 20px 0;
        }
        .demo-box h3 {
            color: #764ba2;
            margin-bottom: 15px;
        }
        
        .user-info {
            background: white;
            border: 3px solid #764ba2;
            border-radius: 12px;
            padding: 25px;
            margin-bottom: 25px;
            box-shadow: 0 4px 12px rgba(118, 75, 162, 0.2);
        }
        .user-info h3 {
            color: #6a1b9a;
            margin-bottom: 15px;
            font-size: 1.3em;
            border-bottom: 2px solid #f3e5f5;
            padding-bottom: 10px;
        }
        .user-info p {
            margin: 10px 0;
            color: #333;
            font-size: 1.05em;
            line-height: 1.8;
        }
        .user-info p strong {
            color: #4a148c;
            font-weight: 600;
        }
        
        .form-group {
            margin-bottom: 15px;
        }
        .form-group label {
            display: block;
            margin-bottom: 8px;
            color: #333;
            font-weight: 500;
        }
        .form-group input {
            width: 100%;
            padding: 10px;
            border: 2px solid #e0e0e0;
            border-radius: 6px;
            font-size: 1em;
        }
        .form-group input:focus {
            outline: none;
            border-color: #764ba2;
        }
        
        .flow-step {
            display: flex;
            align-items: flex-start;
            margin-bottom: 15px;
            padding: 15px;
            background: #f8f9fa;
            border-radius: 8px;
            border-left: 4px solid #764ba2;
        }
        .step-number {
            width: 35px;
            height: 35px;
            background: #764ba2;
            color: white;
            border-radius: 50%;
            display: flex;
            align-items: center;
            justify-content: center;
            font-weight: bold;
            margin-right: 15px;
            flex-shrink: 0;
        }
        .step-content h4 { color: #333; margin-bottom: 5px; }
        .step-content p { color: #666; font-size: 0.9em; margin: 0; }
        
        .tabs {
            display: flex;
            border-bottom: 2px solid #e0e0e0;
            margin-bottom: 20px;
        }
        .tab {
            padding: 12px 20px;
            background: #f8f9fa;
            border: none;
            cursor: pointer;
            transition: all 0.3s;
            font-size: 0.95em;
            color: #666;
        }
        .tab:hover { background: #764ba2; color: white; }
        .tab.active { background: #764ba2; color: white; }
        .tab-content { display: none; }
        .tab-content.active { display: block; }
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            <h1>📤 POST型CSRF攻击演示</h1>
            <p>通过隐藏表单自动提交POST请求，更隐蔽的攻击方式</p>
        </div>
        
        <div class="nav-bar">
            <div class="nav-links">
                <a href="index.php">🏠 首页</a>
                <a href="csrf_get.php">GET型攻击</a>
                <a href="csrf_post.php" class="active">POST型攻击</a>
                <a href="csrf_token_weak.php">Token缺陷</a>
                <a href="csrf_defense.php">防护方案</a>
            </div>
            <div>
                <a href="index.php" class="btn btn-primary">返回首页</a>
                <a href="../index.php" class="btn btn-danger">🏠 返回主页</a>
            </div>
        </div>
        
        <div class="content">
            <?php if (isset($success)): ?>
            <div class="alert alert-success">✅ <?php echo htmlspecialchars($success); ?></div>
            <?php endif; ?>
            
            <?php if (isset($error)): ?>
            <div class="alert alert-danger">❌ <?php echo htmlspecialchars($error); ?></div>
            <?php endif; ?>
            
            <!-- 用户信息 -->
            <div class="user-info">
                <h3>👤 当前用户信息</h3>
                <p><strong>用户名：</strong><?php echo htmlspecialchars($username); ?></p>
                <p><strong>邮箱：</strong><?php echo htmlspecialchars($user_info['email']); ?></p>
                <p><strong>昵称：</strong><?php echo htmlspecialchars($user_info['nickname']); ?></p>
                <p><strong>电话：</strong><?php echo htmlspecialchars($user_info['phone']); ?></p>
            </div>
            
            <!-- 攻击原理 -->
            <div class="section">
                <h2>📚 攻击原理</h2>
                <p>
                    POST型CSRF攻击比GET型更隐蔽，攻击成功率更高。攻击者构造隐藏的HTML表单，
                    通过JavaScript自动提交，用户完全不知情。
                </p>
                
                <div class="code-block">
                    <strong style="color: #764ba2;">// 漏洞代码示例</strong><br>
                    // 直接接受POST参数，无Token验证<br>
                    if ($_SERVER['REQUEST_METHOD'] === 'POST') {<br>
                    &nbsp;&nbsp;$nickname = $_POST['nickname'];<br>
                    &nbsp;&nbsp;$phone = $_POST['phone'];<br>
                    &nbsp;&nbsp;updateUserInfo($nickname, $phone); // 直接更新，无CSRF防护<br>
                    }
                </div>
                
                <div class="alert alert-warning">
                    <strong>⚠️ 危险性：</strong>POST请求不会出现在URL中，更难被发现。且可通过JavaScript自动提交，用户无感知。
                </div>
            </div>
            
            <!-- 攻击流程 -->
            <div class="section">
                <h2>🔄 攻击流程</h2>
                <div class="flow-step">
                    <div class="step-number">1</div>
                    <div class="step-content">
                        <h4>用户登录目标网站</h4>
                        <p>用户正常登录，浏览器保存登录凭证（Cookie）</p>
                    </div>
                </div>
                <div class="flow-step">
                    <div class="step-number">2</div>
                    <div class="step-content">
                        <h4>攻击者构造隐藏表单</h4>
                        <p>创建包含恶意数据的隐藏表单，目标指向目标网站</p>
                    </div>
                </div>
                <div class="flow-step">
                    <div class="step-number">3</div>
                    <div class="step-content">
                        <h4>诱导用户访问恶意页面</h4>
                        <p>通过邮件、社交媒体等方式诱导用户访问攻击页面</p>
                    </div>
                </div>
                <div class="flow-step">
                    <div class="step-number">4</div>
                    <div class="step-content">
                        <h4>自动提交表单</h4>
                        <p>页面加载后，JavaScript自动提交隐藏表单</p>
                    </div>
                </div>
                <div class="flow-step">
                    <div class="step-number">5</div>
                    <div class="step-content">
                        <h4>服务器执行操作</h4>
                        <p>服务器验证用户身份成功，执行修改操作</p>
                    </div>
                </div>
            </div>
            
            <!-- 演示区域 -->
            <div class="section">
                <h2>🎮 演示操作</h2>
                
                <div class="tabs">
                    <button class="tab active" onclick="switchTab('vulnerable')">⚠️ 漏洞版本</button>
                    <button class="tab" onclick="switchTab('secure')">✅ 安全版本</button>
                    <button class="tab" onclick="switchTab('attack')">🔥 攻击演示</button>
                </div>
                
                <!-- 漏洞版本 -->
                <div id="vulnerable" class="tab-content active">
                    <div class="demo-box">
                        <h3>⚠️ 漏洞版本 - 无CSRF防护</h3>
                        <p>以下表单直接提交数据，无任何防护措施：</p>
                        
                        <div class="code-block">
                            <strong style="color: #764ba2;">// 漏洞代码</strong><br>
                            // 直接接受POST参数，无Token验证<br>
                            $nickname = $_POST['nickname'];<br>
                            $phone = $_POST['phone'];<br>
                            $_SESSION['user']['nickname'] = $nickname;<br>
                            $_SESSION['user']['phone'] = $phone;
                        </div>
                        
                        <form method="POST" action="">
                            <input type="hidden" name="action" value="update_vulnerable">
                            <div class="form-group">
                                <label>昵称</label>
                                <input type="text" name="nickname" value="<?php echo htmlspecialchars($user_info['nickname']); ?>">
                            </div>
                            <div class="form-group">
                                <label>电话</label>
                                <input type="text" name="phone" value="<?php echo htmlspecialchars($user_info['phone']); ?>">
                            </div>
                            <button type="submit" class="btn btn-danger">提交修改（漏洞版本）</button>
                        </form>
                        
                        <div class="alert alert-info" style="margin-top: 15px;">
                            <strong>💡 提示：</strong>此表单无CSRF Token，攻击者可以构造相同的表单自动提交。
                        </div>
                    </div>
                </div>
                
                <!-- 安全版本 -->
                <div id="secure" class="tab-content">
                    <div class="demo-box">
                        <h3>✅ 安全版本 - 有CSRF Token防护</h3>
                        <p>使用CSRF Token验证，防止恶意请求：</p>
                        
                        <div class="code-block">
                            <strong style="color: #67c23a;">// 安全代码</strong><br>
                            // 生成Token<br>
                            $_SESSION['csrf_token'] = bin2hex(random_bytes(32));<br><br>
                            // 验证Token<br>
                            if ($_POST['csrf_token'] !== $_SESSION['csrf_token']) {<br>
                            &nbsp;&nbsp;die('CSRF验证失败');<br>
                            }
                        </div>
                        
                        <p><strong>当前CSRF Token：</strong></p>
                        <div class="code-block"><?php echo $_SESSION['csrf_token']; ?></div>
                        
                        <form method="POST" action="">
                            <input type="hidden" name="action" value="update_safe">
                            <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">
                            <div class="form-group">
                                <label>昵称</label>
                                <input type="text" name="nickname" value="<?php echo htmlspecialchars($user_info['nickname']); ?>">
                            </div>
                            <div class="form-group">
                                <label>电话</label>
                                <input type="text" name="phone" value="<?php echo htmlspecialchars($user_info['phone']); ?>">
                            </div>
                            <button type="submit" class="btn btn-success">提交修改（安全版本）</button>
                        </form>
                        
                        <div class="alert alert-success" style="margin-top: 15px;">
                            <strong>✅ 防护效果：</strong>攻击者无法获取有效的Token，恶意请求会被拦截。
                        </div>
                    </div>
                </div>
                
                <!-- 攻击演示 -->
                <div id="attack" class="tab-content">
                    <div class="demo-box">
                        <h3>🔥 攻击演示页面</h3>
                        <p>点击下方按钮打开攻击页面，体验真实的POST型CSRF攻击：</p>
                        
                        <div style="display: flex; gap: 15px; margin-bottom: 20px;">
                            <a href="attack_post.html" target="_blank" class="btn btn-danger">
                                🔥 漏洞版本攻击（成功）
                            </a>
                            <a href="attack_post_safe.html" target="_blank" class="btn btn-success">
                                🛡️ 安全版本攻击（失败）
                            </a>
                        </div>
                        
                        <div class="alert alert-warning" style="margin-top: 20px;">
                            <strong>⚠️ 攻击页面说明：</strong><br>
                            <strong style="color: #f56c6c;">漏洞版本攻击：</strong>攻击页面伪装成"系统升级通知"，实际会自动提交隐藏表单修改您的信息。<br>
                            <strong style="color: #67c23a;">安全版本攻击：</strong>攻击页面尝试攻击有Token防护的表单，但会失败并显示防护效果。<br>
                            请在新标签页打开，然后返回本页面查看信息变化。
                        </div>
                        
                        <div class="code-block">
                            <strong style="color: #f56c6c;">// 漏洞版本攻击代码（成功）</strong><br>
                            &lt;!-- 隐藏的攻击表单 --&gt;<br>
                            &lt;form id="attackForm" action="csrf_post.php" method="POST" style="display:none"&gt;<br>
                            &nbsp;&nbsp;&lt;input type="hidden" name="action" value="update_vulnerable"&gt;<br>
                            &nbsp;&nbsp;&lt;input type="hidden" name="nickname" value="被攻击的用户"&gt;<br>
                            &nbsp;&nbsp;&lt;input type="hidden" name="phone" value="13800138000"&gt;<br>
                            &lt;/form&gt;<br><br>
                            <strong style="color: #67c23a;">// 安全版本攻击代码（失败）</strong><br>
                            &lt;form id="attackForm" action="csrf_post.php" method="POST"&gt;<br>
                            &nbsp;&nbsp;&lt;input type="hidden" name="action" value="update_safe"&gt;<br>
                            &nbsp;&nbsp;&lt;input type="hidden" name="nickname" value="被攻击的用户"&gt;<br>
                            &nbsp;&nbsp;&lt;!-- ❌ 缺少有效的csrf_token --&gt;<br>
                            &nbsp;&nbsp;&lt;input type="hidden" name="csrf_token" value="fake_token"&gt;<br>
                            &lt;/form&gt;<br><br>
                            <strong style="color: #409eff;">// 对比结果</strong><br>
                            漏洞版本：攻击成功，用户信息被修改<br>
                            安全版本：攻击失败，Token验证拦截请求
                        </div>
                    </div>
                </div>
            </div>
            
            <!-- 防护建议 -->
            <div class="section">
                <h2>🛡️ 防护建议</h2>
                <div class="alert alert-success">
                    <strong>✅ 防护措施：</strong>
                    <ul style="margin-left: 20px; margin-top: 10px;">
                        <li>使用CSRF Token验证（推荐）</li>
                        <li>验证HTTP Referer头</li>
                        <li>设置Cookie的SameSite属性</li>
                        <li>关键操作要求二次验证</li>
                        <li>限制表单提交频率</li>
                    </ul>
                </div>
            </div>
            
            <div style="text-align: center; margin-top: 30px;">
                <a href="csrf_token_weak.php" class="btn btn-primary" style="padding: 15px 30px; font-size: 1.1em;">
                    下一课：Token设置缺陷 →
                </a>
            </div>
        </div>
    </div>
    
    <script>
        function switchTab(tabId) {
            // 隐藏所有tab内容
            document.querySelectorAll('.tab-content').forEach(content => {
                content.classList.remove('active');
            });
            // 移除所有tab的active状态
            document.querySelectorAll('.tab').forEach(tab => {
                tab.classList.remove('active');
            });
            // 显示选中的tab
            document.getElementById(tabId).classList.add('active');
            event.target.classList.add('active');
        }
    </script>
</body>
</html>